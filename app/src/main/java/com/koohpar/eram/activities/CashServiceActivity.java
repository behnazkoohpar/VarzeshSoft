...
public class CashServiceActivity extends AppCompatActivity implements View.OnClickListener, IAPIConstantants, IApiUrls {
    ...
    private List<CashService> orderList = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_cash_service);
    ...
    private void showSortDialog() {
        final Dialog dialogSort = new Dialog(CashServiceActivity.this);
                        ...
                        if (response.getString("status").equalsIgnoreCase("true")) {
                            ...
                            for (int i = 0; i < obj.length(); i++) {
                                CashService cashService = new CashService(
                                        ...
                                        obj.getJSONObject(i).getString("ServiceType"),
                                        obj.getJSONObject(i).getString("Title"),
                                        obj.getJSONObject(i).getString("TotalAmount"),
                                        obj.getJSONObject(i).getString("MemberGradeDiscountPercent"),
                                        obj.getJSONObject(i).getString("DiscountPercent"),
                                        obj.getJSONObject(i).getString("DiscountAmount"),
                                        obj.getJSONObject(i).getString("TotalAmountAfterDiscount"),
                                        obj.getJSONObject(i).getString("TaxAndTollPercent"),
                                        obj.getJSONObject(i).getString("TaxAndTollAmount"),
                                        obj.getJSONObject(i).getString("PayableTotalAmount")
                                );
                                orderList.add(cashService);
                            }
                            recyclerView.setAdapter(new CashServiceListRecycleViewAdapter(orderList));
                        } else {
                            CommonMethods.showSingleButtonAlert(CashServiceActivity.this, getString(R.string.txt_attention), response.getString("errmessage"), getString(R.string.pop_up_ok));
                        }
